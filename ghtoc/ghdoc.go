...
func NewGHDoc(Path string, AbsPaths bool, StartDepth int, Depth int, Escape bool, Token string, Indent int, Debug bool, KeepLink bool) *GHDoc {
	return &GHDoc{
		Path:       Path,
		AbsPaths:   AbsPaths,
...
func (doc *GHDoc) IsRemoteFile() bool {
	// 匹配 windows 绝对地址
	if found, _ := regexp.MatchString("^[a-zA-Z]:", doc.Path); found{
